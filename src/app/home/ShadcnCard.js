...
export default function CarouselSize() {
  ...
  useEffect(() => {
    if (products.length > 0) {
      ...
      console.log('selected variants: ', initialVariants);
  ...
  return (
    <div className="relative w-full hidden md:block">
      <Carousel opts={{ align: "start", slidesToScroll: 5 }} className="w-full h-full">
        <CarouselContent className="flex gap-[0.5px] h-fit p-4">
          {Object.keys(selectedVariants).map((productName) => {
            ...
            const numOrder = cartItem ? cartItem.numOrder : 1;

            const selectedVariant = selectedVariants[productName];

            // const discounted_price = selectedVariant.

            return (
              <CarouselItem key={productName} className="md:basis-1/3 lg:basis-1/5 text1 h-full">
                <div className="border hover:scale-105 hover:bg-gradient-to-r hover:from-blue-200 hover:to-white transition-all duration-200 ease-in border-gray-400 rounded-lg bg-white shadow-md hover:shadow-lg flex flex-col h-fit">
                  <div className="relative flex flex-col">
                    ...
                    <div className="p-4">
                      <h2 className="text2 text-md dark:text-white text-gray-600">{selectedVariant?.brand}</h2>

                      <h2
                        className="mb-2 text-base dark:text-white text-gray-900 relative group"
                      ...
                      <div className="flex items-end">
                        <p className="mr-2 text-xl text-gray-900 dark:text-white">
                          ₹{selectedVariant?.discount_lower > 0 ? selectedVariant.discounted_price_lower : selectedVariant?.price}
                        </p>
                        {selectedVariant?.discount_lower > 0 && (
                                  ...
                                  <div className="flex flex-col space-y-1 w-full">
                                    ...
                                    <div className="flex flex-row justify-center items-center space-x-2">
                                      <div className="text-xs bg-green-200 p-1 rounded-lg text-green-500">
                                        <p>{variant.discount_lower}% OFF</p>
                                      </div>
                                      <p className="text-sm">₹{variant.discount_lower > 0 ? variant.discounted_price_lower : variant.price}</p>
                  ...
                  {!cartItem ? (
                    ...
                    <div className="flex justify-center p-3 transition-opacity duration-500 ease-in-out opacity-100 text-lg">
                      <div className="flex flex-row h-[40px] justify-around w-full rounded-lg bg-transparent border-2 border-green-600 text-green-600 hover:bg-green-600 hover:text-white transition-colors duration-300">
                        ...
                        <button>
                          <span className="font-bold">{numOrder}</span>
                        </button>
        ...
        <div className="absolute -top-8 right-[3.2rem] transform -translate-y-1/2">
        ...
        <div className="absolute -top-8 right-[3.5rem] transform -translate-y-1/2">
